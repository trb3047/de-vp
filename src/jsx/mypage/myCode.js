        ...
        try {
            ...
            for (let e in data) {
                ...
                if(tag === pageTag || pageTag === 'ALL') {
                    code += '<li><a href="/myCodeEdit?idx=' + idx + '">'
        ...
        try {
            ...
            if (res.status === 200) {
                ...
                let list = `<p class='blind'>대분류 선택</p>`
                        +  `<a href='/myCode?tag=ALL' class=${(pageTag === 'ALL') ? 'on' : ''}>전체</a>`;
                for (let e in data) {
                    const val = data[e];
                    list += `<a id='tag_${val.name}' href='/myCode?tag=${val.name}' class=${(val.name === pageTag) ? 'on' : ''}>${val.title}</a>`;
                }
                box.innerHTML = list;
            }
        } catch (err) {
        ...
        <React.StrictMode>
            ...
            <main>
                <div className='category text-center'>
                    ...
                    <a href='/myCode?tag=ALL' className='on'>my code</a>
                </div>
                <div id='cont' className='mt-5'>
                    ...
                    <div className='btnGroup text-center mt-3'>
                        <a href='/myCodeAdd' className='btn apply'>code 작성</a>
                    </div>
                </div>
            </main>
            <Footer />
